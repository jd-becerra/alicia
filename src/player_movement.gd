...
@export var tolerance: float = 5.0  # Tolerance for small position changes
...
@onready var initial_pause = true
...
func _physics_process(delta):
	...
	if distance_to_target > stop_distance:
		...
		if abs(global_position.x - last_position.x) < tolerance and direction.x != 0:
			is_moving = false
